function [Rpos_master,DN2V,RMSE_mean,NVpair,RMSE_4m_delVals,eta_path,eta_wire,eta_neuron]=settle_path_length_witn_neu_act(No_roots,Height,Gpos,A,Rpos_master,Level_ID,Npos,actv_inds,epochs,cp_ind,eta_path_max,eta_wire_max,eta_neuron_max,rooted_inds,neural_act)
RMSE_prev=1e15;
stopper=1;
stopper2=1;
...
for path_epochs=1:epochs
    ...
    for nid=1:size(Npos,1)
        ...
        del_2N=-(Rpos_master(Nearest_vessel,:)-Npos(nid,:))*neural_act(nid);
        if numel(find(isnan(del_2N))) > 0
            keyboard;
        end
        NVpair(nid,1)=nid;
...
%     RMSE_Leaf_down(path_epochs,1)=sqrt(mean(mean(Dpl_down(actv_inds)+Dwl_down(actv_inds)+Dn(actv_inds)).^2));
    ...
    % %     if RMSE_4m_delVals(path_epochs,1) < 0.000200
    % %         break;
    % %     end
    % if RMSE_prev<=RMSE_mean(path_epochs,1) || (RMSE_prev-RMSE_mean(path_epochs,1))<1e-12
    % %RMSE_4m_delVals(path_epochs,1)<0.001%|| (RMSE_prev-RMSE_mean(path_epochs,1))<1e-9
    %     if (RMSE_prev-RMSE_mean(path_epochs,1))<1e-12
    %         stopper=stopper+1;
    %     end
    %     if RMSE_prev<RMSE_mean(path_epochs,1) || stopper==100
    % %     close all;
    % % figure(1);subplot(221);plot(RMSE_n); title('Neu');
    % % figure(1);subplot(222);plot(RMSE_pl); title('PL');
    % % figure(1);subplot(223);plot(RMSE_wl);title('WL')
    % % figure(1);subplot(224);plot(RMSE_mean);title('RMSE')
    % %
    % % figure(2);subplot(221);plot(RMSE_n); title('Neu');
    % % figure(2);subplot(222);plot(RMSE_pl_up); title('PL-UP');
    % % figure(2);subplot(223);plot(RMSE_wl_up);title('WL-UP')
    % % figure(2);subplot(224);plot(RMSE_4m_delVals);title(' RMSE 4m delVals')
    % %
    % % figure(3);subplot(221);plot(RMSE_Leaf_down); title('RMSE Leaf down');
    % % figure(3);subplot(222);plot(RMSE_pl_down); title('PL Down');
    % % figure(3);subplot(223);plot(RMSE_wl_down);title('WL Down')
    % % figure(3);subplot(224);plot(RMSE_Leaf_up);title(' RMSE Leaf up')
    % %
    % % figure(4);plot(DN2V_store);title('mean distance from neuron')
    %
    % total_change=sqrt(mean(mean(Rpos_initial-Rpos_master).^2))
    % stopper
    % % keyboard;
    %        break;
    %     end
