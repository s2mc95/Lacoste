...
% load('Control_growth_automated_Stopping_criteria_with_settle_with_step_by_step_wt_41_1S.mat')% 
load('D_BIG_phase2_just_grown_0.043%_Neurons_7S.mat')
%wt_20 <><> 0.55,0.30;
%%
L4Z_max = 0.55;
L4Z_min = 0.30;
token = find(Rpos_master(:,3)<L4Z_max & Rpos_master(:,3)>=L4Z_min);
B_pos=Rpos_master(token,:);
...
if size(slice_details,1)-1 > 0
    for lo=1:size(slice_details,1)
        L_R(lo,1)=sqrt(sum((Rpos_master(slice_details(lo,1),:)-Rpos_master(slice_details(lo,3),:)).^2,2));
    end
...
L_R_split = Volume_proportion(out_g,xx,yy,Rpos_master,NX,L_R_split,L4Z_max,L4Z_min);
...
% load('Lesioned_growth_automated_Stopping_criteria_with_settle_with_step_by_step_wt_38_NS.mat')
% 
% %%
% L4Z_max = 0.55;
% L4Z_min = 0.30;
% token_l = find(Rpos_master(:,3)<L4Z_max & Rpos_master(:,3)>=L4Z_min);
% B_pos_l=Rpos_master(token_l,:);
% 
% %%
...
%     L_R_l(lo,1)=sqrt(sum((Rpos_master(slice_details_l(lo,1),:)-Rpos_master(slice_details_l(lo,3),:)).^2,2));
% end
% r0=0.085;
% kr=1;
% radius_ref0=define_radius(r0,kr,Level_ID);
...
% L_R_split_l = Volume_proportion(out_g_l,xx_l,yy_l,Rpos_master,NX_l,L_R_split_l,L4Z_max,L4Z_min);
