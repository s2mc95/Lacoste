function [Gliapos,Glia_model_distribution,endpt]=distribute_NON_Neu(Length,Width,Height,tissue_Height,file,int)
...
Glia_model_distribution = tsai_window_number / volume_of_tissue;
...
title({[num2str(Length),'mm X ',num2str(Width),'mm X ',num2str(Height),'mm'];['Total Non-neuronal cells=',num2str(tot_neu)]});
...
for den =size(tsai_window_number,1)-1:-1:1
    ...
    Gliapos{cook,1} = [CstNp(:,1),CstNp(:,2),CstNp(:,3)];
